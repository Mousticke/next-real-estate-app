import { ChevronLeft, ChevronRight, Home, LayoutGrid, Map as MapIcon } from "lucide-react";
...
export default async function PropertiesPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const [{ data: properties }, { data: totalCount }, { data: amenities }] = await Promise.all([
    sanityFetch({
    ...
    <div className="min-h-screen bg-accent/20">
      ...
      <div className="bg-background border-b border-border/50">
        <div className="container py-8">
          ...
          <h1 className="text-3xl md:text-4xl font-bold font-heading">Browse Properties</h1>
          <p className="text-muted-foreground mt-2">Find your perfect nest from our curated selection</p>
        </div>
      </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
              <div>
                <p className="text-lg font-medium">
                  <span className="tabular-nums">{totalCount || 0}</span> {totalCount === 1 ? "property" : "properties"} found
                </p>
                {hasFilters && <p className="text-sm text-muted-foreground">Showing filtered results</p>}
              </div>
            </div>

            <Tabs defaultValue="list" className="w-full">
              <TabsList className="mb-6 bg-background border border-border/50 p-1 rounded-xl">
                <TabsTrigger value="list" className="flex items-center gap-2 rounded-lg data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">
                  <LayoutGrid className="h-4 w-4" aria-hidden="true" />
                  Grid View
                </TabsTrigger>
                <TabsTrigger value="map" className="flex items-center gap-2 rounded-lg data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">
              ...
              <TabsContent value="list" className="mt-0">
                {properties && properties.length > 0 ? (
                    ...
                    {totalPages > 1 && (
                      <nav className="flex items-center justify-center gap-2 mt-10" aria-label="Pagination">
                        ...
                        <span className="flex items-center px-4 text-sm text-muted-foreground tabular-nums">
                          Page <span className="font-medium text-foreground mx-1">{page}</span> of <span className="font-medium text-foreground ml-1">{totalPages}</span>
                        </span>
                  ...
                  <div className="text-center py-16 bg-background rounded-2xl border border-border/50">
                    ...
                    <h3 className="text-lg font-semibold font-heading mb-2">No Properties Found</h3>
                    <p className="text-muted-foreground mb-6 max-w-md mx-auto">{hasFilters ? "Try adjusting your filters to see more results." : "There are no properties available at the moment."}</p>
                    {hasFilters && (
