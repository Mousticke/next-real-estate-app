...
/** Mapbox Geocoding API response body (FeatureCollection) */
interface GeocodeResponseBody {
  type: "FeatureCollection";
  features: MapboxFeature[];
}

function isGeocodeResponseBody(body: unknown): body is GeocodeResponseBody {
  if (typeof body !== "object" || body === null || !("features" in body))
    return false;
  const withFeatures = body as { features: unknown };
  return Array.isArray(withFeatures.features);
}
...
export function AddressAutocomplete({
  ...
  const fetchSuggestions = useCallback(async (query: string) => {
    ...
    setIsLoading(true);
    try {
      const baseClient = mapboxSdk({ accessToken: token });
