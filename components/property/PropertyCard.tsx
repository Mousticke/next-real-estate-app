...
export function PropertyCard({ property, onSave, isSaved, showRemoveButton: _showRemoveButton }: PropertyCardProps) {
  ...
  const statusLabel = property.status && property.status !== "active" ? property.status.charAt(0).toUpperCase() + property.status.slice(1) : null;
    ...
    <Link href={`/properties/${property._id}`} className="group block">
      <article className="bg-card text-card-foreground overflow-hidden rounded-2xl border border-border/50 shadow-warm transition-[transform,box-shadow] duration-300 hover:shadow-warm-lg hover:-translate-y-1">
        ...
        <div className="relative aspect-[4/3] overflow-hidden">
          ...
          {/* Status Badge */}
          {statusLabel && (
            <Badge variant={property.status === "sold" ? "destructive" : "muted"} className="absolute top-3 left-3 shadow-sm">
              {statusLabel}
            </Badge>
          )}
          ...
          {property.propertyType && (
            <Badge variant="secondary" className="absolute bottom-3 left-3 capitalize shadow-sm bg-background/90 backdrop-blur-sm">
              {property.propertyType}
            </Badge>
          )}
          ...
          {onSave && (
            ...
            >
              <Heart className={`h-5 w-5 transition-colors duration-200 ${isSaved ? "fill-primary text-primary" : "text-muted-foreground group-hover:text-foreground"}`} aria-hidden="true" />
            </Button>
          )}
        </div>
          ...
          <div className="flex items-center gap-4 text-sm text-muted-foreground mb-4">
            ...
            {property.squareFeet && (
              <div className="flex items-center gap-1.5">
                ...
                <span className="tabular-nums">{property.squareFeet.toLocaleString()}</span>
